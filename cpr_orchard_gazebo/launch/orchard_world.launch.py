...
def generate_launch_description():
    ...
    # Set orchard geometry parameter
    orchard_geom_param = Node(
        package='robot_state_publisher',
        executable='robot_state_publisher',
        name='orchard_geom_publisher',
        parameters=[{
            'robot_description': Command([
                ...
                PathJoinSubstitution([
                    FindPackageShare('cpr_orchard_gazebo'),
                    'urdf',
                    'orchard_geometry.urdf.xacro'
                ])
            ]),
            'use_sim_time': use_sim_time
        }],
        remappings=[('robot_description', 'orchard_geom')]
    )
    ...
    # Spawn orchard world geometry
    spawn_orchard = Node(
        package='gazebo_ros',
        executable='spawn_entity.py',
        name='orchard_gazebo_spawner',
        arguments=[
            '-entity', 'orchard_geometry',
            '-topic', 'orchard_geom',
            '-x', world_x,
            '-y', world_y,
            '-z', world_z,
