...
def generate_launch_description():
    ...
    # Set race geometry parameter
    race_geom_param = Node(
        package='robot_state_publisher',
        executable='robot_state_publisher',
        name='race_geom_publisher',
        parameters=[{
            'robot_description': Command([
                ...
                PathJoinSubstitution([
                    FindPackageShare('cpr_race_modules'),
                    'urdf',
                    'race_geometry.urdf.xacro'
                ])
            ]),
            'use_sim_time': use_sim_time
        }],
        remappings=[('robot_description', 'race_geom')]
    )
    ...
    # Spawn race world geometry
    spawn_race = Node(
        package='gazebo_ros',
        executable='spawn_entity.py',
        name='race_world_spawner',
        arguments=[
            '-entity', 'race_world',
            '-topic', 'race_geom',
            '-x', world_x,
